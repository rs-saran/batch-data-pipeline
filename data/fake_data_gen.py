...
def generate_events_for_one_user(user_id,n):
    ...
    user_total_days = random.randint(0, 300)  # Total days the user has activity
    user_activity_days = sorted([random_date(start_date, end_date) for _ in range(user_total_days)])
    ...
    for day in user_activity_days:
        ...
        while day_total_consumption < daily_consumption_efficiency*total_seconds_in_a_day:
            ...
            session_end = session_start + timedelta(seconds=next_session_duration)
            adid = str(uuid.uuid4())
            ...
            session_end_events.append({
                ...
                "sessionid": session_id,
                "appversion": random.choice(['1.0', '1.1', '1.2']),
                "event_specific_properties": {"session_duration":next_session_duration}  # No specific properties for session_end
            })

            consumption_efficieny = random.uniform(0, 1)
            session_total_consumption = 0
            next_consumption_duration = random.randint(0, 7200) #first consumption duration
            ...
            while session_total_consumption < next_session_duration*consumption_efficieny :
                
                # Pick a content that was launched before or on the session day
                valid_content = content_df[content_df['release_date'] <= session_start]
                ...
                consumption_end = consumption_start + timedelta(seconds=next_consumption_duration)
                consumption_rate = random.uniform(0, 1)  # Random rate of completion
                ...
                consumption_start_events.append({
                    ...
                    "event_specific_properties": {
                        ...
                        "consumption_type": random.choice(['online', 'downloaded']),
    ...
    print()
    print(f"Completed generating data for {n}th user with {len(user_activity_days)} days in {elapsed_time:.4f} seconds")
