...
with DAG(
    ...
    with TaskGroup("unnest_session_jobs_group") as unnest_session_jobs_group:
        for task_name in ["session_start", "session_end"]:
            unnest_session_job = BashOperator(
                task_id=f"unnest_job_{task_name}",
                bash_command=f"{bash_command_prefix} {task_name} {{ds}}",
            )
    with TaskGroup(
        "unnest_consumption_jobs_group"
    ) as unnest_consumption_jobs_group:
        for task_name in ["consumption_end", "consumption_start"]:
            unnest_consumption_job = BashOperator(
                task_id=f"unnest_job_{task_name}",
                bash_command=f"{bash_command_prefix} {task_name} {{ds}}",
            )
